...
largura = 1080
altura = 450
tela = pygame.display.set_mode((largura,altura), pygame.RESIZABLE) #cria a tela do jogo
...
while rodando:
    ...
    if not jogo_iniciado:
      ...
      tela.blit(iniciar, (posicaox_iniciar, posicaoy_iniciar))
        ...
        recorde_na_tela = fonte.render(f'RECORDE: {int(maior_recorde)}', False, (0,255,0))
        ...
        bruno_colisao = pygame.Rect(bruno_x, bruno_y, bruno_01.get_width()/2, bruno_01.get_height()/2) 
        ...
        if bruno_colisao.colliderect(salada_colisao) or bruno_colisao.colliderect(anilha_colisao):
        ...
        if int(pontos) // 200 > ultimo_marco:
            ...
            dificuldade = min(dificuldade, 3.0) #limite
